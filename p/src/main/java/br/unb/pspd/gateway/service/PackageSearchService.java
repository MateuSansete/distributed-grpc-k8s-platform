...
/**
 * Orquestra a busca de pacotes: chama FlightService (A) e HotelService (B) em
...
public class PackageSearchService {
    ...
    public PackageSearchResponseDto search(String origin, String destination,
        ...
        // 1) Chama A e B EM PARALELO num executor dedicado.
        ...
        CompletableFuture<List<Hotel>> hotelsFuture = CompletableFuture.supplyAsync(
        ...
        for (Flight f : flights) {
            for (Hotel h : hotels) {
                evaluated++;
                long total = f.getPrice().getAmountCents()
                        + h.getPricePerNight().getAmountCents() * nights * travelers;
                String currency = f.getPrice().getCurrency();
                packages.add(new TravelPackageDto(
                        "PKG-" + f.getFlightId() + "-" + h.getHotelId(),
                        FlightDto.from(f),
                        null, // return_flight: apenas ida nesta entrega
                        HotelDto.from(h),
                        (int) nights,
